...
if __name__ == "__main__" :
    ...
    incoming_purge_message_handler = MessageHandler(
        ...
        filters=filters.command(["purge@Devilvideo2bot"]) & filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    incoming_g_clear_handler = MessageHandler(
        g_clearme,
        filters=filters.command(["renewme@Devilvideo2bot"]) & filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    help_text_handler = MessageHandler(
        help_message_f,
        filters=filters.command(["help@Devilvideo2bot"]) & filters.chat(chats=AUTH_CHANNEL)
    )
